...
`test_server.c` теперь выступает как канонический `server-first` пример и поднимает маршруты:
...
`test_server.c` больше не пытается демонстрировать все utility-функции и условные возможности сразу; файл сфокусирован на базовом сценарии: `http_init` -> `http_register_route` -> `http_listen` -> `http_run`.

Канонический пример в `README.md` синхронизирован с `test_server.c` и должен обновляться вместе с ним.

`test_client.c` не тестирует библиотечный клиент, а использует внешний `libcurl` для обращения к локальному серверу. То есть это вспомогательный demo-клиент для сервера, а не пример использования собственного HTTP client API из `libs/http`.
